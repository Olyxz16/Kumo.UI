...
def _parse_color(expr: str) -> RGBA:
    ...
    if s.startswith("color-mix("):
        ...
        if space in ("in oklch", "in oklab"):  # mix in Oklab space
...
def emit_axaml(tokens: list[Token]) -> str:
    lines = [
...
def emit_md(tokens: list[Token]) -> str:
    out = [
        ...
        "",
        f"{len(tokens)} tokens. Set `RequestedThemeVariant` (Light/Dark) to switch.",
        "",
...
def main() -> int:
    ...
    if not tokens:
        ...
        return 1
    axaml, md = emit_axaml(tokens), emit_md(tokens)
